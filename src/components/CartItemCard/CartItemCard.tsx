...
const CartItemCard = ({ item, handleDeleteCartItem }: Props) => {
    ...
    const handleChangeQuantity = useCallback(async (newQuantity: number) => {
        if (newQuantity < 1) return; // Prevent invalid quantity
        setIsUpdating(true);
        try {
            const res = await cartApi.updateCart({ productId: item.productId, quantity: newQuantity });
            if (res.status === 200) {
        ...
        <Grid container sx={{ mx: 3, my: 2 }} spacing={2}>
            <Grid item xs={3}>
                <img
                    ...
                    height={120}
                    width={'100%'}
                    src={"https://www.crunchlabs.com/cdn/shop/files/preview_images/97fd59f272a44c32887a4920f79a1519.thumbnail.0000000000.jpg?v=1715981102&width=150"} // Use the correct property for the image URL
            ...
            <Grid item xs={9} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <TextField
                    value={item.quantity}
                    ...
                    InputProps={{
                        endAdornment: (
                            <InputAdornment position="end">
                                <IconButton onClick={handleIncrement} disabled={isUpdating}>
                        ...
                        startAdornment: (
                            <InputAdornment position="start">
                                <IconButton onClick={handleDecrement} disabled={item.quantity === 1 || isUpdating}>
